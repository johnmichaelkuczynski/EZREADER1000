...
- February 3, 2025: CONTENT SOURCE FIXES - Fixed critical content source functionality: (1) Added content source support to homework mode across all LLM providers, (2) Enhanced dialogue system to include content source context, (3) Created new "Query Content Source" feature allowing users to ask questions directly about uploaded content with dedicated input box and instant answers.
- September 1, 2025: HUMANIZER WORKFLOW IMPROVEMENTS - Fixed critical paste functionality in Box A and Box B by restructuring dropzone overlays to not interfere with normal text input. Added "Send to Humanizer" button directly in output box toolbar for seamless workflow. Fixed Perplexity API model name. Added Azure OpenAI support to schema validation. GPTZero detection confirmed active on both Box A and Box C with automatic triggering after text changes.
- September 1, 2025: COMPLETE HUMANIZER OVERHAUL - Successfully implemented complete style-matching logic replacing old "humanization" approach. Created new aiProviders service with buildRewritePrompt function handling 4-case logic (Box B/Box C combinations). Added comprehensive philosophical writing samples and instruction presets. Updated /api/humanize-text endpoint to use pure style-matching instead of asking LLMs to "humanize". System now tells LLMs to "rewrite Box A content so it exactly matches the granular style of Box B content" with no humanization instructions. Confirmed working with dramatic AI detection score reductions (0.99→0.001).
